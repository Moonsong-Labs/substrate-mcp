...
impl ReleaseAnalyzer {
    ...
    pub async fn analyze_release(&self, release: &str) -> Result<ReleaseAnalysis> {
        ...
        // Generate summary
        let summary = self
            .generate_summary(&pr_data, &index, &categories, &impact)
            .await?;
    ...
    async fn load_pr_data(&self, release: &str) -> Result<Vec<PrData>> {
        ...
        if !pr_data_dir.exists() {
            return Err(anyhow!(
                "PR analysis data directory not found: {}",
                pr_data_dir.display()
            ));
        }
    ...
    fn assess_risk(&self, pr: &PrData) -> RiskLevel {
        if pr.patch_analysis.has_migrations {
            ...
            || pr
                ...
                .affected_files
                .iter()
                .any(|f| f.contains("consensus") || f.contains("crypto") || f.contains("security"))
        {
            RiskLevel::High
